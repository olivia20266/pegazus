...
type User = { id:string; first_name:string; last_name:string; email:string; country:string|null; kyc_status:string; status:string; role:string; phone?:string|null; birth_date?:string|null; nationality?:string|null; learning_id?:string|null; wallets:{ balance:number; free_margin:number; learning_balance:number; mt5_login:string }[]; created_at:string }
      ...
      <aside style={{ width:220, background:'#0c0f1a', borderRight:'1px solid rgba(255,255,255,.07)', padding:'22px 0', display:'flex', flexDirection:'column', flexShrink:0 }}>
        <div style={{ padding:'0 18px 22px', borderBottom:'1px solid rgba(255,255,255,.07)', marginBottom:14 }}>
          <div style={{ display:'flex', alignItems:'center', gap:10 }}>
            <div style={{ width:34, height:34, borderRadius:8, background:'linear-gradient(135deg,#d4a843,#f0c96a)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:15, fontWeight:700, color:'#06080e' }}></div>
            <div>
                  ...
                  <tbody>
                    {filtered.map(u=>(
                      <tr key={u.id} style={{ borderBottom:'1px solid rgba(255,255,255,.03)' }}>
                        ...
                        {page==='wallets' ? (
                          <>
                            <td style={{ padding:'12px 16px', fontFamily:"'DM Mono',monospace", fontSize:12, color:'#8a96aa' }}>{u.wallets?.[0]?.mt5_login||'—'}</td>
                            <td style={{ padding:'12px 16px', fontFamily:"'DM Mono',monospace", fontSize:13, color:'#2dd4a0', fontWeight:600 }}>{fmt((u.wallets?.[0]?.balance||0))}</td>
                            <td style={{ padding:'12px 16px', fontFamily:"'DM Mono',monospace", fontSize:13, color:'#5a677d' }}>{fmt((u.wallets?.[0]?.free_margin||0))}</td>
                            <td style={{ padding:'12px 16px', fontFamily:"'DM Mono',monospace", fontSize:13, color:'#d4a843' }}>{fmt((u.wallets?.[0]?.learning_balance||0))}</td>
                            <td style={{ padding:'12px 16px' }}>
                          ...
                          <>
                            ...
                            <td style={{ padding:'12px 16px', fontSize:13 }}>{u.country||'—'}</td>
                            <td style={{ padding:'12px 16px' }}>{badge(u.kyc_status)}</td>
                            <td style={{ padding:'12px 16px', fontFamily:"'DM Mono',monospace", fontSize:13, color:'#2dd4a0' }}>{fmt((u.wallets?.[0]?.balance||0))}</td>
          ...
          <div style={{ background:'#0c0f1a', border:'1px solid rgba(255,255,255,.08)', borderRadius:18, width:'100%', maxWidth:460, maxHeight:'90vh', overflow:'auto' }}>
            <div style={{ padding:'22px 26px 18px', borderBottom:'1px solid rgba(255,255,255,.07)', display:'flex', justifyContent:'space-between', alignItems:'flex-start' }}>
              <div>
                ...
                <div style={{ fontSize:13, color:'#5a677d', marginTop:3 }}>{adjUser.first_name} {adjUser.last_name} — actuel : <strong style={{ color:'#d4a843' }}>{fmt((adjUser.wallets?.[0]?.balance||0))}</strong></div>
              </div>
            ...
            <div style={{ padding:'22px 26px' }}>
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10, marginBottom:16 }}>
                {(['credit','debit'] as const).map(t=>(
                  <div key={t} onClick={()=>setAdjType(t)} style={{ border:`2px solid ${adjType===t?(t==='credit'?'#2dd4a0':'#f0544f'):'rgba(255,255,255,.07)'}`, borderRadius:11, padding:14, textAlign:'center', cursor:'pointer', background: adjType===t?(t==='credit'?'rgba(45,212,160,.07)':'rgba(240,84,79,.07)'):'transparent' }}>
                    ...
                    <div style={{ fontSize:11, color:'#5a677d', marginTop:2 }}>{t==='credit'?'Augmenter':'Réduire'}</div>
                  </div>
                ))}
              </div>
              <div style={{ background: adjType==='credit'?'rgba(45,212,160,.06)':'rgba(240,84,79,.06)', border:`2px solid ${adjType==='credit'?'rgba(45,212,160,.3)':'rgba(240,84,79,.3)'}`, borderRadius:12, padding:18, textAlign:'center', marginBottom:16 }}>
                ...
                <div style={{ fontSize:12, color:'#5a677d', marginTop:4 }}>
                  {'Après : '}<strong style={{ color:'#edf0f7' }}>{fmt(Math.max(0,((adjUser.wallets?.[0]?.balance||0))+(adjType==='credit'?1:-1)*parseFloat(adjAmt||'0')))}</strong>
                </div>
              </div>
