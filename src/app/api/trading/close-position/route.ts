...
export async function POST(req: NextRequest) {
  ...
  if (!session) return NextResponse.json({ error: 'Non autorisÃÂÃÂÃÂÃÂ©' }, { status: 401 })
  ...
  // 1. RÃÂÃÂÃÂÃÂ©cupÃÂÃÂÃÂÃÂ©rer le wallet actuel
  ...
  // 2. Mettre ÃÂÃÂÃÂÃÂ  jour le wallet (balance + learning_balance en mÃÂÃÂÃÂÃÂªme temps)
  ...
  await supabaseAdmin.from('transactions').insert({
    ...
    status:       'COMPLETED',
    source:       symbol,
    description:  `Trade fermÃÂÃÂÃÂÃÂ© ${type} ${lot} lot ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ ${reason} (${isWin ? 'gain' : 'perte'})`,
  ...
  // 4. Notification admin (entrÃÂÃÂÃÂÃÂ©e dans audit_logs pour que l'admin voie)
  ...
  try {
    ...
    // Ne pas bloquer si vertex est indisponible ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ on logue seulement
